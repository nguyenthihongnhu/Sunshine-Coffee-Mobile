<?php 
	...
	class Database{
		...
		public function __construct(){
			$this->link = mysqli_connect("localhost", "root", "", "sunshine_coffee") or die ();
		...
		public function updateview($sql){
			$result = mysqli_query($this->link,$sql) or die ("Lỗi update update view" .mysqli_error($this->link));
		...
		public function deletewhere($table, $data = array()){
			foreach($data as $id){
				$id = intval($id);
				$sql = "DETELE FROM {$table} WHERE id = $id ";
				mysqli_query($this->link,$sql) or die ("Lỗi truy vấn delete ---" .mysqli_error($this->link));
			}
			return true;
		}
		...
		public function fetchIDMuti($table,$query, $id){
		...
		public function fetchAllTable($table,$query){
		...
		// 	$result = mysql_query($this->link,$sql) or die ("Lỗi truy vấn fetchJone ---" .mysql_error($this->link));
